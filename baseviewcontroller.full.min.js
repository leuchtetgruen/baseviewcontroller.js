...
_.each(fcts,function(fctName){that[fctName]=function(){return elem[fctName].apply(elem,arguments);};});},getClassByName:function(name){if(name.match(/[A-Za-z0-9]+/)){return eval(name);}
...
var ContentView=function(elem,vc,mixins){var myMixins={initialize:function(){this._setContentAccordingToViewFillWith();},updateContents:function(){this._setContentAccordingToViewFillWith();},_setContentAccordingToViewFillWith:function(){var viewFillWith=$(this.element).attr("view-fill-with");if(viewFillWith&&vc[viewFillWith]){viewFillWithResult=vc[viewFillWith];if(typeof(viewFillWithResult)=="function"){this.set(viewFillWithResult.apply(vc));}
else{this.set(viewFillWithResult);}}},};mergedMixins=_.extend(mixins||{},myMixins);return HTMLable(elem,vc,mergedMixins);};var ViewTypes={"contentview":ContentView,"container":Container,};var BaseViewController=Backbone.View.extend({transitionTo:function(ViewControllerClass,element,data){if(data&&data['empty']){_doHide=data['empty'];data=_.omit(data,'empty');}
else _doHide=false;this.hide(_doHide);var extendedData=_.extend({fromViewController:this},data);var MixedVcClass=ViewControllerClass.extend(extendedData);newVc=new MixedVcClass({el:element});newVc._updateUrl();},transitionBack:function(){if(this.fromViewController){this.hide();this.fromViewController.show();this.fromViewController._updateUrl();}},_hide:function(doEmpty){var that=this;this.animateHide(function(){that.$el.hide();if((doEmpty==undefined)||(doEmpty))that.$el.empty();});},_show:function(){window.visibleViewController=this;window.$el=this.$el;this.$el.show();this.$el.css("z-index",1000);this.animateShow();},animateShow:function(callback){if(this.hasAnimationProperty()){visibleClass=this.getAnimationVisibleClass();invisibleClass=this.getAnimationInvisibleClass();this.$el.removeClass("animatable");this.$el.removeClass(visibleClass);this.$el.addClass(invisibleClass);var that=this;window.setTimeout(function(){that.$el.bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd",function(){that.$el.removeClass(invisibleClass);that.$el.unbind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd");if(callback)callback();});that.$el.addClass("animatable");that.$el.addClass(visibleClass);},50);}
...
$(elem).addClass("active-tab");var MixedVcClass=viewControllerClass.extend({parentViewController:vc});new MixedVcClass({el:$(that.parentContainer.contentElement)});});}};mergedMixins=_.extend(mixins||{},myMixins);return HTMLable(elem,vc,mergedMixins);};ViewTypes["tabview"]=TabView;ViewTypes["tabitem"]=TabItem;var BackLink=function(elem,vc,mixins){var myMixins={initialize:function(){if(vc.fromViewController){$(this.element).click(function(e){vc.transitionBack();e.preventDefault();});}
else{$(this.element).hide();}},};mergedMixins=_.extend(myMixins,mixins);return HTMLable(elem,vc,mergedMixins);};ViewTypes["backlink"]=BackLink;String.prototype.hashCode=function(){var hash=0;if(this.length==0)return hash;for(i=0;i<this.length;i++){char=this.charCodeAt(i);hash=((hash<<5)-hash)+char;hash=hash&hash;}
return hash;}
RouterBuilder=function(){this._routes=[]
this.addRoute=function(routeDescription,routeFunction){routeHash={};routeHash[routeDescription]=routeFunction;this._routes.push(routeHash);};this.registerRouteForViewController=function(routeDescription,ViewControllerClass,element){this.addRoute(routeDescription,function(){if(window.visibleViewController){window.visibleViewController.transitionTo(ViewControllerClass,element);}
else{new ViewControllerClass({el:element});}});};this.setDefaultRoute=function(ViewControllerClass,element){this.registerRouteForViewController("*actions",ViewControllerClass,element);};this.registerRouteWithCollections=function(routeDescription,ViewControllerClass,element,paramToCollectionHash){if(!paramToCollectionHash)paramToCollectionHash={};var that=this;routeFunction=function(){var params=_.filter(arguments,function(arg){return(arg!=null);});var sortedParams=that._sortParametersAccordingToAppearanceInRouteDescription(routeDescription,paramToCollectionHash);var extension={};var next=function(idx){if(idx>=params.length){var MixedVcClass=ViewControllerClass.extend(extension);if(window.visibleViewController){window.visibleViewController.transitionTo(MixedVcClass,element);}
else{new MixedVcClass({el:element});}
return;}
paramName=sortedParams[idx];ItemClass=paramToCollectionHash[paramName];if(ItemClass){var item=new ItemClass();item.id=params[idx];item.fetch({success:function(){extension[paramName]=item;next(idx+1);},});}};next(0);};this.addRoute(routeDescription,routeFunction);};this.build=function(){_routes=this._buildRoutesHash();var RouterClass=Backbone.Router.extend({routes:_routes,});routerInstance=new RouterClass();this._registerRouteCallbacks(routerInstance);window.appRouter=routerInstance;return routerInstance;};this._buildRoutesHash=function(){routesHash={};_.each(this._routes,function(route){routeDescription=_.keys(route)[0];routesHash[routeDescription]=routeDescription.hashCode();});return routesHash;};this._registerRouteCallbacks=function(router){_.each(this._routes,function(route){routeDescription=_.keys(route)[0];routeCallback=route[routeDescription];router.on("route:"+routeDescription.hashCode(),routeCallback);});};this._sortParametersAccordingToAppearanceInRouteDescription=function(routeDescription,paramHash){retHash=_.clone(paramHash);retPositions=_.mapObject(paramHash,function(val,key){return routeDescription.indexOf(key);});retPositionsPairs=_.pairs(retPositions);sortedPairs=_.sortBy(retPositionsPairs,function(pair){return pair[1];});sortedPairMappedIndizes=_.map(sortedPairs,function(pair){return pair[0];});return sortedPairMappedIndizes;};this._getClassByName=function(name){if(name.match(/[A-Za-z0-9]+/)){return eval(name);}
else return null;};};